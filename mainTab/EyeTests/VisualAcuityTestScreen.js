...
import { View, Text, TouchableOpacity, StyleSheet, SafeAreaView, Image, Button, Alert } from 'react-native';
...
export default function VisualAcuityTestScreen({ navigation, route }) { 
  ...
  const [eSymbolSize, setESymbolSize] = useState(50); // Initial size for the E symbol
  ...
  const getReferenceBoxWidth = () => {
    return selectedEye === 'left' ? REFERENCE_BOX_WIDTH_LEFT_EYE : REFERENCE_BOX_WIDTH_RIGHT_EYE;
  };
  ...
  const captureFrame = async () => {
    try {
        ...
        if (frameCount % 30 === 0 && cameraRef.current) { // Capture every 20th frame
  ...
  const uploadFrame = async (base64Image) => {
    try {
      const response = await fetch('http://192.168.57.214:5000/detect', {
        method: 'POST',
...
const renderEyeStatus = () => {
  ...
  const isStartButtonEnabled = () => {
    ...
    const isEyeDetectionCorrect = 
      (selectedEye === 'right' && detectedObjects.includes('right_eye') && !detectedObjects.includes('left_eye')) ||
      (selectedEye === 'left' && detectedObjects.includes('left_eye') && !detectedObjects.includes('right_eye'));

    // Check if the preferred distance is met
    const isDistanceCorrect = distance && preferredDistance && distance >= preferredDistance;
  ...
  const generateRandomDirections = () => {
    ...
    const randomDirectionsArray = Array.from({ length: 10 }, () => directionsArray[Math.floor(Math.random() * directionsArray.length)]);
...
const handleArrowPress = (direction) => {
  if (currentDirectionIndex < randomDirections.length) {
    ...
    setESymbolSize(prevSize => Math.max(prevSize - 5, 20)); // Prevent the size from getting too small
  }

  // When all directions have been tested
  if (currentDirectionIndex === randomDirections.length && randomDirections.length > 0) {
...
const calculateScore = async (finalScore) => {
  try {
    ...
    const response = await fetch('https://eyecheck-flask-svm.onrender.com/predict', {
      method: 'POST',
      body: JSON.stringify({
        age,
        eyeglasses,
        ...
        right_eye_score: rightEyeScore !== null ? rightEyeScore : finalScore,
      }),
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.backButton} onPress={handleBack}>
        <Text style={styles.backButtonText}>
          ‹ {showEyeTestSelection || showCameraView || showTestImages ? 'Back' : 'Visual Acuity Test'}
        </Text>
      </TouchableOpacity>
      ...
      {!showEyeTestSelection ? (
        <View style={styles.content}>
          <Text style={styles.description}>
            Good eye coordination requires healthy vision. We focus on two tests: visual acuity and astigmatism. Conduct these tests at 40 cm from a mobile screen or 1 meter from a computer screen. Keep glasses or contacts on if you wear them. Remember, this isn't a replacement for a full eye exam by an optometrist. If needed, schedule one.
          </Text>
        ...
        <View style={styles.eyeTestSelectionContent}>
          ...
          <Text style={styles.instruction}>Which eye do you want to test first?</Text>
          <View style={styles.buttonContainer}>
            <TouchableOpacity style={styles.button} onPress={() => handleEyeSelection('right')}>
              <Text style={styles.buttonText}>LEFT</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.button} onPress={() => handleEyeSelection('left')}>
        ...
        <View style={styles.eyeTestContent}>
          <Text style={styles.testTitle}>{selectedEye.toUpperCase()} EYE TEST</Text>
            ...
            <View style={styles.row}>
              <View style={styles.column}>
                <TouchableOpacity onPress={() => handleArrowPress('up')}>
                  <Image source={require('../../assets/arrow_up.png')} style={styles.arrowImage} />
                </TouchableOpacity>
              </View>
            </View>
            <View style={styles.row}>
              <View style={styles.column}>
                <TouchableOpacity onPress={() => handleArrowPress('left')}>
                  <Image source={require('../../assets/arrow_left.png')} style={styles.arrowImage} />
                </TouchableOpacity>
              </View>
              <View style={styles.column}>
                <Image source={getESymbolImage().source} style={[styles.eImage, { width: eSymbolSize, height: eSymbolSize }]} />
              </View>
              <View style={styles.column}>
                <TouchableOpacity onPress={() => handleArrowPress('right')}>
                  <Image source={require('../../assets/arrow_right.png')} style={styles.arrowImage} />
                </TouchableOpacity>
              </View>
            </View>
            <View style={styles.row}>
              <View style={styles.column}>
                <TouchableOpacity onPress={() => handleArrowPress('down')}>
                  <Image source={require('../../assets/arrow_down.png')} style={styles.arrowImage} />
                </TouchableOpacity>
              </View>
            </View>
          </View>
          <Text style={styles.instruction}>
            Click the arrow keys to indicate which direction the E symbol is facing.
          </Text>
        </View>
        ...
        <View style={styles.cameraContainer}>
          ...
          <Text style={styles.instruction}>
            Keep both eyes open and cover the {selectedEye}.
            Focus on the E symbol.
            Click the arrow keys to indicate which direction the E symbol is facing.
          </Text>
          <TouchableOpacity
            style={[styles.continueButton, !isStartButtonEnabled() && styles.disabledButton]}
